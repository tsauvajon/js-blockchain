...
{
  var ft = doTransaction(-1, INIT_ADDRESS, wf.CONF.START_TOKEN);
  return ft;
}
...
global.VERIFY_ADDRESS = function(_addr)
{
  ...
  if(typeof _addr != "string") return false;
  if(_addr.indexOf(PROTOCOL) != 0)
  {
    return false;
  }
  if(_addr.length != INIT_ADDRESS.length)
  {
    return false;
  }
  return true;
